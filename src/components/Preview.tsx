...
class Preview extends React.Component<PreviewProps> {
  constructor(props: PreviewProps) {
    ...
    this.loadObserver = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) this.onLoad();
      },
      {
        rootMargin: "100%",
      }
  ...
  file(): TreeNode {
    if (this.props.cwd.type & this.props.types.FILE) return this.props.cwd;
  ...
  fetchFile(): void {
    ...
    if (!(file.type & this.props.types.FILE)) return;
  ...
  generateFileInfo(name: string, lines: number, size: number): JSX.Element {
    return (
      <div className="file-info">
        <a
          id={`file-info-actions.${this.getIndex(this.file())}`}
          className="file-edit"
          onClick={this.props.handleNodeActionClick}
        >
          {name}
        </a>
  ...
  isEditable(node: TreeNode | FileNode): boolean {
    return !(
      node.type & this.props.types.IMAGE || node.type & this.props.types.VIDEO
    );
  }
  ...
  > {
    return {
      ...
      link: (props) => {
        ...
        if (cwd[0] === "root") cwd = cwd.slice(1, cwd.length);
        ...
        return (
          <StorageLink
            ...
            href={href}
            children={children}
            callback={this.props.linkCallback}
          />
        );
      },
    };
  ...
  generateFilePreview(): JSX.Element | null {
    ...
    if (cwd.type & this.props.types.CRAWLABLE) {
      const nodes = cwd.children
        .map((node) => {
          if (
            node.type & this.props.types.IMAGE ||
    ...
    if (!(cwd.type & this.props.types.FILE)) return null;
      ...
      return (
        <CodeBlock
          text={
            ...
            (this.state.data.data[this.state.data.data.length - 1] === "\n"
              ? " "
              : "")
          }
          language={
            this.state.data.type & this.props.types.RAW
  ...
  generateCodeEditor(): JSX.Element {
    return (
      <div className="editor" ref={this.targetRef}>
        ...
        />
        <textarea
          ref={this.dummyHighlightRef}
          className="editor-extra-style"
          name="data"
          id="data"
          value={this.data(this.state.data.data)}
          readOnly={true}
        ></textarea>
        <textarea
          ref={this.dummyRef}
  ...
  onScroll(): void {
    ...
    if (this.targetRef.current) {
      if (this.dummyRef.current) {
        (
          this.targetRef.current.childNodes[0].childNodes[1] as HTMLElement
        ).scrollLeft = this.dummyRef.current.scrollLeft;
        this.dummyRef.current.style.height =
          (this.targetRef.current.childNodes[0].childNodes[1] as HTMLElement)
            .offsetHeight + "px";
        this.dummyRef.current.style.width =
          (this.targetRef.current.childNodes[0].childNodes[1] as HTMLElement)
            .offsetWidth + "px";
      }

      if (this.dummyHighlightRef.current) {
        this.dummyHighlightRef.current.scrollLeft = (
          this.targetRef.current.childNodes[0].childNodes[1] as HTMLElement
        ).scrollLeft;
        this.dummyHighlightRef.current.style.height =
          (this.targetRef.current.childNodes[0].childNodes[1] as HTMLElement)
            .offsetHeight + "px";
        this.dummyHighlightRef.current.style.width =
          (this.targetRef.current.childNodes[0].childNodes[1] as HTMLElement)
            .offsetWidth + "px";
      }
    }
  ...
  run(): void {
    ...
    try {
      ...
      err = (thrownError as Error).stack;
    }
  ...
  render(): JSX.Element {
    if (!this.state.file) return <div className="file"></div>;
    ...
    return (
      <div ref={this.props.innerRef} className="file">
          ...
          <div className="header">
            {this.props.isEditing
              ? this.generateFileInfo(
                  ...
                  new TextEncoder().encode(this.state.data.data).length
                )
              : this.generateFileInfo(
                  this.state.data.file,
            ...
            <div
              id={`file-preview-actions.${this.getIndex(this.file())}`}
              className="file-actions"
              onClick={this.props.handleNodeActionClick}
            >
              {this.props.isEditing === false &&
              this.file().extension === "js" ? (
                <a className="file-run" onClick={() => this.run()}>
                  <RunIcon />
                </a>
              ) : null}
              {this.isEditable(this.file()) ? (
                this.props.isEditing ? (
                  <a
                    className="file-save"
                    onClick={() =>
                      this.props.onEdit(this.file(), this.state.data.data)
                    }
                  >
                    <SaveIcon />
                  </a>
              ...
              {this.props.isEditing ? (
                <a
                  className="file-save file-cancel"
                  onClick={() =>
                    this.props.onEdit(this.file(), this.state.data.initial)
                  }
                >
                  <CancelIcon />
                </a>
        ...
        <div className="file-preview">
          {this.props.isEditing === false && this.file().extension === "js" ? (
            this.state.run.file === this.file() ? (
              <div className="run-output">
                Output:<div className="run-result">{this.state.run.res}</div>
                <div className="run-error">{this.state.run.err}</div>
              </div>
            ) : null
          ) : null}
          {this.props.isEditing
          ...
          {this.props.isEditing && this.file().extension === "md"
            ? this.generateFilePreview()
            : null}
        </div>
      </div>
...
export default React.forwardRef<HTMLElement, PreviewProps>((props) => (
  <Preview {...props} />
));
