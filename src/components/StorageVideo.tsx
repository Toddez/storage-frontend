...
class StorageVideo extends React.Component<StorageVideoProps> {
  ...
  async onLoad(): Promise<void> {
    ...
    if (this.state.shouldLoad) {
      ...
      if (!this.state.shouldPlay) await this.ref.current.pause();
  ...
  async onPlay(): Promise<void> {
    ...
    if (this.state.shouldPlay) await this.ref.current?.play();
    else await this.ref.current?.pause();
  }
      ...
      >
        <video
          ...
          className={`${this.state.className}${
            this.state.shouldPlay ? " playing" : " paused"
          }`}
        ></video>
