...
class Explorer extends React.Component<Props> {
    ...
    const elements = (
      <div className="path">
        ...
        {this.state.position.map((value, index) => {
          ...
          node = next;
          const isDir =
            node.type & this.state.types.DIR ||
            index + 1 < this.state.position.length;

          return (
            <div id={`path-item.${index + 1}`} className="item" key={index + 1}>
              <a onClick={this.handlePathClick}>{value}</a>
              {isDir ? "/" : ""}
            </div>
          );
        })}
  ...
  generateCreateModal(): JSX.Element | null {
    if (!this.state.displayCreate) return null;
  ...
  generateUploadModal(): JSX.Element | null {
    if (!this.state.displayUpload) return null;

    return (
      <UploadModal
        hide={() => this.setState({ displayUpload: false })}
        submit={(data) => this.upload(data)}
      />
    );
  }

  generateUploadURLModal(): JSX.Element | null {
    if (!this.state.displayUploadURL) return null;

    return (
      <UploadURLModal
        hide={() => this.setState({ displayUploadURL: false })}
        submit={(data) => this.uploadFromURL(data as Record<string, string>)}
      />
    );
  }

  treeName(node: TreeNode): string {
    if (this.state.editingName === node) return this.state.name;
  ...
  generateNode(node: TreeNode, index: number): JSX.Element {
    ...
    const filePattern =
      "(([.]{0,1}|([.][/]){0,1}|([.][.][/]){0,1})[A-Za-z0-9]+)+([A-Za-z0-9]|([./][A-Za-z0-9]))*";
    const dirPattern = "[A-Za-z0-9]+([A-Za-z0-9]|([/][A-Za-z0-9])|([/]$))*";

    let pattern = "";
    if (this.state.editingName)
      pattern =
        this.state.editingName.type & this.state.types.FILE
          ? filePattern
          : dirPattern;
    ...
    return (
      <div className="node" key={index}>
        ...
        <div
          id={`tree-node-actions.${index}`}
          className="actions"
          onClick={this.handleNodeActionClick}
        >
          <a className="file-rename">Rename</a>
  ...
  generateNodes(): JSX.Element {
    ...
    return (
      <React.Fragment>
        <div
          className={
            "collapse-nodes" + (this.state.collapsed ? " collapsed" : "")
          }
          onClick={() => {
        ...
        <div className="nodes">
          {node !== this.state.tree ? (
            <div className="node go-up" onClick={this.handleTreeClick}>
              <input
                type="text"
                className="name"
                value=".."
  ...
  handlePathClick(event: ClickEvent<HTMLAnchorElement>): void {
    ...
    if (this.state.displayCreate) return;

    if (this.state.displayUpload) return;

    const target = event.target;
    const value = target.text;
    const id = parseInt(target.parentElement.id.split(".")[1]);

    if (value === "~") return this.navigate("");
  ...
  onEdit(node: TreeNode, data: string): void {
    ...
    if (path[0] === "root") path = path.slice(1, path.length);
  ...
  handleNodeActionClick(event: ClickEvent<HTMLDivElement>): void {
    ...
    while (target.parentElement) {
      ...
      if (classList[0] === "file-rename") action = "rename";

      if (classList[0] === "file-edit") action = "edit";

      if (classList[0] === "file-delete") action = "delete";
  ...
  handleTreeClick(event: ClickEvent<HTMLInputElement>): void {
    ...
    if (value === "..")
      return this.navigate(
        this.state.position.slice(0, this.state.position.length - 1).join("/")
      );
      ...
      <div className="Explorer">
        <div className="popups">
          {this.state.popups.map((v, i) => (
            <div
              key={i}
              className={`popup ${v.done ? "done" : "pending"} ${
                v.ok == null ? "" : v.ok ? "ok" : "error"
              }`}
            >
              {v.message}
            </div>
                ...
                {this.currentNode().type & this.state.types.DIR ? (
                  <React.Fragment>
                    <div
                      ...
                      onClick={() => this.displayCreateFolderModal()}
                    >
                      <a>
                        <NewFolderIcon />
                      </a>
                    ...
                    <div
                      className="add-file"
                      onClick={() => this.displayCreateFileModal()}
                    >
                      <a>
                        <NewFileIcon />
                      </a>
                    ...
                    <div
                      ...
                      onClick={() => this.displayUploadModal()}
                    >
                      <a>
                        <UploadIcon />
                      </a>
                    ...
                    <div
                      className="upload-file"
                      onClick={() => this.displayUploadURLModal()}
                    >
                      <a>
                        <CloudIcon />
                      </a>
        ...
        <Preview
          innerRef={this.previewRef}
          cwd={this.currentNode()}
          types={this.state.types}
          handleNodeActionClick={this.handleNodeActionClick}
          isEditing={this.state.editingFile}
          onEdit={this.onEdit}
          linkCallback={this.onLink}
